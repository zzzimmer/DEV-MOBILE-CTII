...
async function carregarPorCategoria(categoria) {
  ...
  try{
    ...
    //Defone as URLs conforme a categoria selecionada
    if (categoria === 'todos'){
    ...
    produtos.forEach(produto => {
      ...
      card.className = "col";
      //a linha produto.image.replace(".jpg", "t.png") serve como um tratamento de dados que estavam vindo da api em formato errado.
      card.innerHTML = `
    ...
    navegar('tela-home')
    // ele ainda funciona, mas assim ainda tem um erro... estranho
    // navegar(tela-home)

  } catch (error){
    console.log("Erro ao carregar a os produtos",error
    )
  }
...
//async permite que você use await dentro dela!! 
async function abrirDetalhes(id){
  ...
  try {
    ...
    <div class = "row g-3>
      ...
      <p><strong>Preço:</strong>${p.price.toFixed(2)}</p>
